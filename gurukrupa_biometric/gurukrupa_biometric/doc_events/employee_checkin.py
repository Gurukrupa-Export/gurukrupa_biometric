...
def set_unique_id():
	...
	for i in data_list:
		try:
			...
			if frappe.db.get_value('Employee Checkin',{'time':datetime.strftime(datetime_obj,"%Y-%m-%d %H:%M:%S"),'employee':employee_name},'unique_id'): 
				continue
			...
			frappe.db.set_value('Employee Checkin',emp_record,'unique_id',i['id'])
		except:
			continue
